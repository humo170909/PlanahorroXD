...
// La Edge Function supabase/functions/log-login-attempt/ es OPCIONAL
// (solo agrega la IP real al registro de auditoría de login). Por
// defecto está en `false` a propósito: si se llamara siempre sin
// haberla desplegado, cada intento de login generaría una petición
// que responde 404 (visible en la consola del navegador), aunque el
// código ya tenga un respaldo que funciona igual. Cambia esto a
// `true` únicamente DESPUÉS de correr
// `supabase functions deploy log-login-attempt`.
export const LOGIN_AUDIT_EDGE_FUNCTION_ENABLED = false;
