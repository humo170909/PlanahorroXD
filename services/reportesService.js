...
// Usada por dashboard.js (Fase 4) y por reportes.js (Fase 9).
...
export async function getGastosPorCategoriaMesActual(userId) {
  const { data, error } = await supabase
    .from('view_gastos_por_categoria_mes_actual')
    .select('*')
    .eq('user_id', userId)
